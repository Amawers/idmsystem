 ...
 * - Create/update records directly in Supabase.
 ...
 *   names and different record shapes coming from table rows vs. fetched records.
 */
...
/**
 * Normalizes a date-like input to `YYYY-MM-DD` (for DATE columns and `<input type="date">`).
 * @param {any} v
 * @returns {string | null}
 */
...
/**
 * Normalizes a datetime-like input into a `<input type="datetime-local">` string.
 * @param {any} v
 * @returns {string}
 */
...
/**
 * @typedef {Object} IntakeSheetCiclcarEditProps
...
/**
 * @param {IntakeSheetCiclcarEditProps} props
 * @returns {JSX.Element}
 */
export default function IntakeSheetCICLCAREdit({
	open,
	setOpen,
	row,
	onSuccess,
	...
	/**
	 * Prefills store sections when opening with an existing record.
	 *
	 * Also fetches family background rows from Supabase when online and needed.
	 * @returns {void}
	 */
	useEffect(() => {
		if (open && row) {
			...
			setSectionField("violationOfCICLCar", {
				...
				admissionDate:
					normalizeDate(row.violation_admission_date) || "",
			...
			// Fetch and prefill family background rows from a related table when needed.
			const applyFamilyData = async () => {
				try {
					if (
						Array.isArray(row.family_background) &&
						row.family_background.length
					) {
						setSectionField(
							"familyBackground",
							"members",
					...
					if (error) {
						console.error(
							"Error fetching family background:",
							error,
						);
						return;
					}
	...
	/**
	 * Creates/updates the CICL/CAR case and related family background rows.
	 *
	...
	const handleCreate = async () => {
		try {
			...
			console.log(
				isEditing
					? "✏️ Updating CICL/CAR record"
					: "🔍 Creating new CICL/CAR record",
				all,
			);
			...
			let targetCaseId = row?.id ?? null;

			if (isEditing && targetCaseId) {
				...
				const { error: deleteFamilyError } = await supabase
					.from("ciclcar_family_background")
					.delete()
					.eq("ciclcar_case_id", targetCaseId);
				...
				targetCaseId = createdCase?.id ?? null;
			}

			if (targetCaseId && familyRows.length > 0) {
				const normalizedFamilyRows = familyRows.map((member) => ({
					ciclcar_case_id: targetCaseId,
			...
			if (onSuccess) {
				try {
					...
					console.error(
						"CICL/CAR edit onSuccess callback failed:",
						callbackError,
					);
				}
			}
			...
			toast.error(isEditing ? "Update Failed" : "Creation Failed", {
				description:
					err.message || "An error occurred while saving the case.",
			});
		} finally {
			setIsSaving(false);
		}
	...
	/**
	 * Advances to the next tab; on the last tab, saves the record.
	 * @returns {Promise<void>}
	 */
	const goNext = async () => {
	...
	/**
	 * Resets the active tab when the dialog closes.
	 * @returns {void}
	 */
	useEffect(() => {
		if (!open) {
	...
	/**
	 * Only reset the store on close to avoid wiping prefilled values on open.
	 * @returns {void}
	 */
	useEffect(() => {
		if (!open) {
