/**
 * Notifications hook (offline-capable).
 *
 * Responsibilities:
 * - Aggregate notifications from:
 *   - audit logs (`fetchAuditLogs`)
 *   - inventory alerts (`inventory_alerts`)
 *   - computed deadline reminders (cases/enrollments)
 * - Cache the merged list + read state in `localStorage` for offline viewing.
 * - Poll while online and update state without requiring realtime subscriptions.
 *
 * Design notes:
 * - Auth-category audit logs are intentionally filtered out to avoid showing
 *   sign-in/sign-out activity in the user-facing UI.
 * - Caching is best-effort; failures to persist/parse should not break the UI.
 */
 ...
 * @property {string} id Unique id for the UI list (may be derived).
 * @property {string} title Short label.
 * @property {string} message Human-readable message.
 * @property {string} createdAt ISO timestamp.
 * @property {NotificationSeverity|string} severity
 * @property {string} category
 * @property {string} actor
 * @property {string|null} resourceLabel
 * @property {any} raw Source row/payload.
 */

/**
 * @typedef {Object} UseNotificationsOptions
 * @property {number} [limit=20] Max audit-log notifications to fetch.
 */

/**
 * @typedef {Object} RefreshNotificationsOptions
 * @property {boolean} [silent=false] When true, does not toggle the loading spinner.
 */
 ...
 * @property {boolean} hasCachedData
 * @property {string|null} lastSyncedAt
 * @property {(id: string) => void} markAsRead
 * @property {() => void} markAllAsRead
 * @property {(options?: RefreshNotificationsOptions) => Promise<void>} refreshNotifications
 * @property {(id: string) => boolean} isNotificationUnread
 */
...
const buildNotification = (log) => {
	...
	return {
		id: log.id,
		title: toTitleCase(
			log.action_type || log.action_category || "Activity",
		),
...
const normalizeSeverity = (value) => {
	...
	if (severity === "info" || severity === "medium" || severity === "low")
		return "info";
	return "info";
};
...
const chooseCaseThreshold = (priority) => {
	...
	return (
		CASE_STALE_THRESHOLDS_DAYS[key] ?? CASE_STALE_THRESHOLDS_DAYS.default
	);
};
...
const mapInventoryAlertToNotification = (alert) => {
	...
	return {
		...
		resourceLabel: alert.item_name
			? `Inventory • ${alert.item_name}`
			: "Inventory",
		raw: alert,
	};
};
	...
	if (isSocialWorker || fullName) {
		...
		(caseRows || []).forEach((row) => {
			...
			const updatedAt =
				...
				safeDate(row.identifying_intake_date) ||
	...
	// These are true deadline dates and map well to reminders/alerts.
	{
		...
		(enrollments || []).forEach((row) => {
			...
			reminders.push({
				...
				category,
				actor: "System",
				resourceLabel: row.case_id
					? `Enrollment • ${String(row.case_id).slice(0, 8)}`
					: "Enrollment",
				raw: {
					kind: "enrollment_deadline",
...
const filterAuditLogsForRole = (logs, _role) => {
	...
	return (logs || []).filter((log) => {
		if (!log) return false;
		const category =
			typeof log.action_category === "string"
				? log.action_category.toLowerCase()
				: "";
...
const filterOutAuthNotifications = (items = []) =>
	(items || []).filter(
		(item) =>
			!isAuthCategory(item?.category) &&
			!isAuthCategory(item?.raw?.action_category),
	);
...
/**
 * Expose notification data sourced from audit logs, alerts, and reminders.
 * @param {UseNotificationsOptions} [options]
 * @returns {UseNotificationsResult}
 */
export function useNotifications({ limit = 20 } = {}) {
	...
	useEffect(() => {
		...
		const cachedReadIds = safeParse(
			window.localStorage.getItem(READ_KEY),
			[],
		);
		...
		const handleStorage = (event) => {
			if (event.key === CACHE_KEY && event.newValue) {
				setNotifications(
					filterOutAuthNotifications(safeParse(event.newValue, [])),
				);
			}
	...
	const refreshNotifications = useCallback(
		async ({ silent = false } = {}) => {
			...
			try {
				...
				const filteredAudit = filterAuditLogsForRole(
					data || [],
					profileContext.role,
				);
				const mapped = filteredAudit.map(buildNotification);
				const alertNotifications = (alertsResult?.data || []).map(
					mapInventoryAlertToNotification,
				);
				const reminders = Array.isArray(remindersResult?.data)
					? remindersResult.data
					: [];

				setNotifications((prev) => {
					const merged = mergeNotifications(
						[...mapped, ...alertNotifications, ...reminders],
						prev,
						limit,
					);
	...
	const unreadCount = useMemo(() => {
		...
		return notifications.reduce(
			(count, notification) =>
				readIds.has(notification.id) ? count : count + 1,
			0,
		);
