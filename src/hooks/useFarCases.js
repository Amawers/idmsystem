...
export function useFarCases() {
  ...
  const load = useCallback(async () => {
    ...
    try {
      const { data: rows, error: err } = await runSupabaseQueryWithTimeout(
        ...
        {
          timeoutMessage:
            "Loading FAR records timed out. Please try refresh again.",
        },
      );
      if (err) throw err;
  ...
  const deleteFarCase = useCallback(async (caseId) => {
    try {
      const { error: err } = await supabase.from("far_case").delete().eq("id", caseId);
      if (err) throw err;
      await load();
  ...
  return useMemo(() => ({ data, loading, error, reload: load, deleteFarCase, pendingCount, syncing, syncStatus, runSync }), [data, loading, error, load, deleteFarCase, pendingCount, syncing, syncStatus, runSync]);
}
