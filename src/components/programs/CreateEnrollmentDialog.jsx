 ...
 */
export default function CreateEnrollmentDialog({ open, onOpenChange, onSuccess }) {
    ...
    try {
      ...
      const formattedCases = (data || []).map(caseItem => {
        ...
        if (caseType === 'FAC') {
          ...
          displayName = `IVAC - ${new Date(caseItem.reporting_period).toLocaleDateString()}`;
        } else {
  ...
  const getAvailablePrograms = () => {
    ...
    return programs.filter(program => {
      ...
      const targetsType = program.target_beneficiary?.includes(formData.case_type);
                  ...
                  {searchingCase ? (
                    ...
                    >
                      <SelectTrigger id="case_id">
                        <SelectValue placeholder={cases.length === 0 ? "No active cases found" : "Select a case"} />
                      </SelectTrigger>
                      <SelectContent>
                        {cases.map(caseItem => (
                          <SelectItem key={caseItem.id} value={caseItem.id}>
                            {caseItem.displayName} - {caseItem.case_manager}
                          </SelectItem>
                        ))}
                      </SelectContent>
                    </Select>
