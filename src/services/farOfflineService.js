/**
 * Offline-first helpers for FAR cases.
 *
 * Responsibilities:
 * - Cache FAR cases in Dexie for offline reads.
...
/**
 * @typedef {Object} FarCaseRecord
 * @property {string} [id] Supabase id (may be missing for offline-created rows)
 * @property {string} [created_at]
 * @property {string} [updated_at]
 * @property {FarLocalMeta} [local]
 */

/**
 * @typedef {Object} FarQueueOp
 * @property {"create"|"update"|"delete"} operationType
 * @property {number} targetLocalId Dexie primary key for `far_cases`
...
const TABLE_NAME = "far_case";
const CASE_TABLE = offlineCaseDb.table("far_cases");
const QUEUE_TABLE = offlineCaseDb.table("far_queue");
...
export async function getFarPendingOperationCount() {
	return QUEUE_TABLE.count();
}

/**
 * Fetches a cached FAR case by Supabase `id`.
 * @param {string} targetId
 * @returns {Promise<any|null>}
 */
export async function getFarCaseById(targetId) {
...
/**
 * Fetches a cached FAR case by Dexie `localId`.
 * @param {number} localId
 * @returns {Promise<any|null>}
 */
export async function getFarCaseByLocalId(localId) {
 ...
 */
export async function loadFarRemoteSnapshotIntoCache() {
...
/**
 * Creates or updates a local FAR case and enqueues an operation.
 ...
 */
export async function createOrUpdateLocalFarCase({
	casePayload,
	targetId = null,
	localId = null,
	mode = "create",
}) {
	if (!casePayload) throw new Error("Missing FAR case payload");
	await localIdReady;
 ...
 */
export async function markFarLocalDelete({ targetId = null, localId = null }) {
	await localIdReady;
	return offlineCaseDb.transaction(
		"rw",
		CASE_TABLE,
 ...
 */
export async function deleteFarCaseNow({ targetId = null, localId = null }) {
	await localIdReady;
	...
	const fallback = await markFarLocalDelete({
...
let farSyncInFlight = null;

/**
 * Replays queued FAR operations against Supabase.
 ...
 */
export function syncFarQueue(statusCb) {
	if (farSyncInFlight) {
		return farSyncInFlight;
	}

	farSyncInFlight = (async () => {
		await localIdReady;
		...
		for (const op of operations) {
			try {
				...
				if (op.operationType === "create") {
					...
					if (!op.targetId) {
						throw new Error(
							"Cannot update FAR case without Supabase id",
						);
					}
					const { data, error } = await supabase
						.from(TABLE_NAME)
