 ...
 * @param {Object} rawData - Object with cases, ciclcar, fac, ivac arrays
...
export async function saveRawDataToCache(dashboardType, rawData) {
    try {
        ...
        if (existing) {
            await offlineCaseDb.dashboard_raw_data.update(existing.id, cacheEntry);
        } else {
 ...
 */
export async function getDashboardData(dashboardType = 'case', filters = {}, forceRefresh = false) {
