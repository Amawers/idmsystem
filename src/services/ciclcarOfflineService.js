/**
 * Offline-first service for CICL/CAR cases.
 *
 * Responsibilities:
 * - Cache case rows and related family background members in Dexie (IndexedDB).
 * - Stage local mutations into a FIFO queue for eventual sync to Supabase.
 * - Preserve local edits (`hasPendingWrites`) when refreshing remote snapshots.
 * - Ensure every cached row has a stable `localId` used as Dexie primary key.
 *
 * Design notes:
 * - `localId` is backfilled on startup via `ensureLocalIdField()`.
 * - Sync replays queued operations in order and stops on first error.
 * - Family background is synced using a replace-all strategy for each case.
 */
...
/**
 * @typedef {Object} CiclcCarLocalMeta
...
/**
 * @typedef {Object} CiclcCarFamilyMemberUi
...
/**
 * @typedef {Object} CiclcCarFamilyMemberPayload
...
/**
 * @typedef {Object} CiclcCarQueueOp
 * @property {"create"|"update"|"delete"} operationType
 * @property {number} targetLocalId
 * @property {string|null} targetId
 * @property {{case: Object, family_background: CiclcCarFamilyMemberPayload[]}|null} payload
 ...
 * This can occur after retries or interrupted snapshot loads.
 ...
 * @param {Object} [member]
 * @returns {CiclcCarFamilyMemberUi}
 */
 ...
 * @param {Object} [member]
 * @returns {CiclcCarFamilyMemberPayload}
 */
...
/**
 * LiveQuery stream of CICL/CAR cases ordered by `localId` (newest first).
	...
	await offlineCaseDb.transaction("rw", CASE_TABLE, async () => {
		...
		for (const row of rows) {
			...
			const record = buildLocalRecord(row, {
				family_background: (familyMap.get(row.id) ?? []).map(
					toUiFamilyMember,
				),
			});
			if (existing?.localId) {
...
/**
 * Loads a full remote snapshot (cases + family background) into the local cache.
...
/**
 * Creates or updates a local CICL/CAR case and enqueues a sync operation.
 * @param {Object} params
 ...
 */
export async function deleteCiclcarCaseNow({
	targetId = null,
	localId = null,
}) {
	await localIdReady;
...
/**
 * Syncs family background members for a case using a replace-all strategy.
 * @param {string} caseId
 * @param {CiclcCarFamilyMemberPayload[]} [familyMembers]
 * @returns {Promise<void>}
 */
...
/**
 * Replays queued CICL/CAR operations to Supabase in FIFO order.
 * Stops at first error to preserve queue ordering.
 *
 * @param {(info: {current: CiclcCarQueueOp, synced: number}) => void} [statusCb]
