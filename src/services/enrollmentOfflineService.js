/**
 * Offline-first helpers for program enrollments.
 *
 * Responsibilities:
 * - Cache enrollment rows in Dexie (IndexedDB) for offline reads.
 * - Stage local mutations into a FIFO queue for eventual sync to Supabase.
 * - Hydrate/enrich enrollments with the related program record via `ENROLLMENT_SELECT`.
 * - Cache case/program snapshots for dropdowns when online.
 *
 * Design notes:
 * - Local-only metadata (e.g., `hasPendingWrites`) is stripped before syncing.
 * - Sync replays queue operations in order and stops on first failure.
 */
...
/**
 * @typedef {Object} EnrollmentLocalMeta
 ...
 * @property {string} [id] Supabase id (may be missing for local-only rows)
 ...
 * @property {number} [sessions_absent_unexcused]
 * @property {number} [sessions_absent_excused]
 * @property {EnrollmentLocalMeta} [local]
 */

/**
 * @typedef {Object} EnrollmentQueueOp
 * @property {"create"|"update"|"delete"} operationType
 * @property {number} targetLocalId Dexie primary key for `program_enrollments`
...
/**
 * Normalizes numeric fields in an enrollment row for consistent UI rendering.
 * @param {Object} [row]
 * @returns {Object}
 */
...
/**
 * Removes local-only metadata and server-managed fields before writing to Supabase.
...
/**
 * Builds a local cache record with default local metadata fields.
 * @param {Object} base
...
/**
 * LiveQuery stream of enrollments ordered by `enrollment_date` (newest first).
 * @returns {import('dexie').LiveQuery<any[]>}
 */

export const enrollmentsLiveQuery = () =>
	liveQuery(async () => {
		const rows = await ENROLLMENTS_TABLE.orderBy("enrollment_date")
			.reverse()
			.toArray();
 ...
 * @param {EnrollmentQueueOp} operation
...
/**
 * Replaces cached enrollments with a remote snapshot while preserving locally edited rows.
...
async function replaceAllCachedEnrollments(rows = []) {
	await offlineCaseDb.transaction("rw", ENROLLMENTS_TABLE, async () => {
		const existingPending = await ENROLLMENTS_TABLE.where(
			"hasPendingWrites",
		)
			.equals(1)
			.toArray();
...
/**
 * Replaces cached enrollments for a single program.
...
/**
 * Fetches enrollments from Supabase and refreshes the local cache.
...
/**
 * Fetches enrollments for a given programId and refreshes the local cache slice.
 * @param {string} programId
 * @returns {Promise<{success: boolean, offline?: true, count?: number}>}
 */
...
/**
 * Creates or updates a local enrollment and stages a queue operation.
 * @param {Object} payload
 * @param {string|null} [enrollmentId]
 * @returns {Promise<{success: true, localOnly: true}>}
 */

export async function createOrUpdateLocalEnrollment(
	payload,
	enrollmentId = null,
) {
	...
	await offlineCaseDb.transaction(
		"rw",
		[ENROLLMENTS_TABLE, ENROLLMENT_QUEUE_TABLE],
		async () => {
			let localId;

			if (isUpdate) {
				const existing = await ENROLLMENTS_TABLE.where("id")
					.equals(enrollmentId)
					.first();
				if (!existing) {
...
/**
 * Marks an enrollment as deleted locally and enqueues a delete operation.
...
/**
 * Deletes an enrollment immediately when online; otherwise falls back to a queued delete.
 * Also cleans up any queued operations for the enrollment when a remote delete succeeds.
 * @param {string} enrollmentId
 * @returns {Promise<{success: true, deletedFromServer?: true, localOnly?: true}>}
 */
...
/**
 * Replays queued enrollment operations to Supabase in FIFO order.
 * Stops at first failure to avoid re-ordering side effects.
 * @param {(message: string) => void | null} [statusCallback]
 * @returns {Promise<{success: boolean, offline?: true, synced?: number, errors?: Array<{operation: any, error: string}>}>}
 */
...
export async function syncEnrollmentQueue(statusCallback = null) {
	...
	const operations =
		await ENROLLMENT_QUEUE_TABLE.orderBy("createdAt").toArray();
	...
	for (const op of operations) {
		try {
			if (statusCallback) {
				statusCallback(
					`Syncing ${op.operationType} operation ${synced + 1}/${operations.length}...`,
				);
			}
...
/**
 * Caches VAC cases in the dedicated VAC cache table.
...
/**
 * Caches FAR cases in the dedicated FAR cache table.
 * @param {Array<any>} cases
 * @returns {Promise<void>}
 */
...
/**
 * Returns locally cached case rows by case type.
 * @param {EnrollmentCaseType|string} caseType
...
/**
 * Fetches a limited list of cases for dropdowns and caches the result.
 * Uses different Supabase tables and filters per case type.
 * @param {EnrollmentCaseType|string} caseType
 * @returns {Promise<{success: boolean, offline?: true, count?: number, data?: Array<any>, error?: string}>}
 */
export async function fetchAndCacheCasesByType(caseType) {
	...
	try {
		...
		if (error) {
			console.error(
				`[Enrollment] Error fetching ${caseType} from ${tableName}:`,
				error,
			);
			throw error;
		}
		...
		console.log(
			`[Enrollment] Fetched ${data?.length || 0} ${caseType} cases from ${tableName}`,
		);
		...
		if (caseType === "VAC") {
			...
			await offlineCaseDb.transaction(
				...
				async () => {
					...
					const existing = await CICLCAR_CASES_TABLE.where(
						"hasPendingWrites",
					)
						.equals(1)
						.toArray();
					...
					if (data?.length) {
						await CICLCAR_CASES_TABLE.bulkPut(
							data.map((c) => ({
								...c,
								hasPendingWrites: false,
							})),
			...
			console.log(
				`[Enrollment] Cached ${data?.length || 0} CICL/CAR cases`,
			);
			...
			await offlineCaseDb.transaction(
				...
				IVAC_CASES_TABLE,
				async () => {
					const existing = await IVAC_CASES_TABLE.where(
						"hasPendingWrites",
					)
						.equals(1)
						.toArray();
					...
					if (data?.length) {
						await IVAC_CASES_TABLE.bulkPut(
							data.map((c) => ({
								...c,
								hasPendingWrites: false,
							})),
...
/**
 * Fetches active programs from Supabase and refreshes the local cache.
 * @returns {Promise<{success: boolean, offline?: true, count?: number, error?: string}>}
 */
export async function fetchAndCachePrograms() {
	...
	try {
		...
		await offlineCaseDb.transaction("rw", PROGRAMS_TABLE, async () => {
			...
			if (data?.length) {
				await PROGRAMS_TABLE.bulkPut(
					data.map((p) => buildLocalRecord(p, {})),
				);
			}
		});
