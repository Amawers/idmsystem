/**
 * Incidence on VAC (IVAC) submission helpers.
 *
 * Responsibilities:
 * - Validate store-backed IVAC form state.
 * - Map form state to the Supabase `ivac_cases` table shape.
 * - Provide CRUD helpers (create/update/fetch/list/delete) with audit logging.
 * - Provide a small statistics helper to derive aggregates from stored records.
 */
...
/**
 * @typedef {Object} IvacBarangayRecord
 * @property {string} [barangay]
 * @property {number|string} [vacVictims]
 * @property {number|string} [genderMale]
 * @property {number|string} [genderFemale]
 * @property {number|string} [age0to4]
 * @property {number|string} [age5to9]
 * @property {number|string} [age10to14]
 * @property {number|string} [age15to17]
 * @property {number|string} [age18Plus]
 * @property {number|string} [physicalAbuse]
 * @property {number|string} [sexualAbuse]
 * @property {number|string} [psychologicalAbuse]
 * @property {number|string} [neglect]
 * @property {number|string} [violenceOthers]
 */

/**
 * @typedef {Object} IvacFormSection
 * @property {string} [province]
 * @property {string} [municipality]
 * @property {IvacBarangayRecord[]} [records]
 * @property {string[]} [caseManagers]
 * @property {string[]} [case_managers]
...
/**
 * @typedef {Object} IvacFormData
 * @property {IvacFormSection} [incidenceOnVAC]
 */

/**
 * @typedef {Object} IvacValidationResult
 * @property {boolean} valid
 * @property {string[]} errors
 */

/**
 * @typedef {Object} IvacCasePayload
 * @property {string} province
 * @property {string} municipality
 * @property {IvacBarangayRecord[]} records
 * @property {string[]} case_managers
 ...
 * A loose representation of an `ivac_cases` row returned by Supabase.
 * @property {string} id
 * @property {string} [province]
 * @property {string} [municipality]
 * @property {IvacBarangayRecord[]} [records]
...
/**
 * @typedef {Object} IvacStatistics
 * @property {number} totalCases
 * @property {number} totalVacVictims
 * @property {Record<string, number>} byBarangay
 * @property {{ male: number, female: number }} byGender
 * @property {{ "0-4": number, "5-9": number, "10-14": number, "15-17": number, "18+": number }} byAge
 * @property {{ physical: number, sexual: number, psychological: number, neglect: number, others: number }} byViolenceType
 */

/**
 * Validate IVAC form data.
 * @param {IvacFormData} formData Store-backed form data.
 * @returns {IvacValidationResult}
 */
...
/**
 * Build the `ivac_cases` insert/update payload from the intake store shape.
 * @param {IvacFormData} [formData]
 * @returns {IvacCasePayload}
 */
export function buildIVACCasePayload(formData = {}) {
	...
	return {
		...
		reporting_period:
			ivacData.reportingPeriod || ivacData.reporting_period || null,
 ...
 * @param {IvacFormData} formData Complete form data from the intake store.
	...
	try {
		...
		if (!data || !data.id) {
			const noDataError = new Error(
				"No data returned from Supabase after insert",
			);
...
export async function updateIVACCase(caseId, formData) {
	try {
		...
		if (!data) {
			const noDataError = new Error(
				"No data returned from Supabase after update",
			);
			console.error("❌", noDataError.message);
 ...
 * @returns {Promise<{data: IvacCaseRow|null, error: Error|null}>}
 */
...
/**
 * Map database record to the intake store form-data structure.
 * @param {IvacCaseRow} dbRecord Database record from Supabase.
 * @returns {IvacFormData|null}
 */
 ...
 *  offset?: number,
 * }} [filters]
 * @returns {Promise<{data: IvacCaseRow[]|null, error: Error|null, count: number}>}
 */
export async function fetchAllIVACCases(filters = {}) {
	try {
		...
		console.log(
			`✅ Fetched ${data?.length || 0} IVAC cases (total: ${count || 0})`,
		);
...
export async function bulkDeleteIVACCases(caseIds) {
	try {
		if (!caseIds || !Array.isArray(caseIds) || caseIds.length === 0) {
			const error = new Error(
				"Case IDs array is required for bulk deletion",
			);
			console.error("❌", error.message);
...
/**
 * Compute IVAC statistics from stored records.
 *
 * Note: This fetches `records` arrays and aggregates totals client-side.
 * @param {{ status?: string, startDate?: string, endDate?: string }} [filters]
 * @returns {Promise<{data: IvacStatistics|null, error: Error|null}>}
 */
	...
	try {
		...
		data.forEach((caseRecord) => {
			...
			records.forEach((record) => {
				...
				stats.byBarangay[record.barangay] += parseInt(
					record.vacVictims || 0,
					10,
				);
				...
				// By violence type
				stats.byViolenceType.physical += parseInt(
					record.physicalAbuse || 0,
					10,
				);
				stats.byViolenceType.sexual += parseInt(
					record.sexualAbuse || 0,
					10,
				);
				stats.byViolenceType.psychological += parseInt(
					record.psychologicalAbuse || 0,
					10,
				);
				stats.byViolenceType.neglect += parseInt(
					record.neglect || 0,
					10,
				);
				stats.byViolenceType.others += parseInt(
					record.violenceOthers || 0,
					10,
				);
			});
		});
